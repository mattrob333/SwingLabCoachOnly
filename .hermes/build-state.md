...
4. [~] AI: Deepgram transcription worker, OpenAI packaging (coach voice preserved), approval flow — **IN PROGRESS**: transcription adapter layer ✅ (types + MockTranscriptionAdapter + DeepgramTranscriptionAdapter + env-gated factory, 18 tests), **transcription worker route ✅** (POST /api/submissions/[id]/transcribe — auth + ownership + transcribe + update manifest note if exists, 9 tests, 617 total), **OpenAI packaging adapter layer ✅** (types + MockPackagingAdapter + OpenAIPackagingAdapter + env-gated factory — 6th application of the env-gated adapter pattern; guardrail-enforcing system prompt; deterministic mock titles/summary; 36 tests, 653 total), **packaging worker route ✅** (POST /api/submissions/[id]/package — auth + ownership + load manifest + call packaging adapter + persist aiSummary + aiNoteTitles onto manifest; adds aiNoteTitles field to LessonPlaybackManifest; 8 tests, 661 total), **coach edit AI output ✅** (PATCH /api/submissions/[id]/package — coach reviews/edits aiSummary + aiNoteTitles on manifest; partial updates preserve other field; aiNoteTitles validated against manifest note IDs; 11 tests, 672 total), **coach approval route ✅** (POST /api/submissions/[id]/approve — transitions manifest status to "approved" + triggers delivery token + email on first approval; idempotent on re-approve; 409 if manifest not packaged; non-fatal side effects; 9 tests, 681 total), **coach AI review panel ✅ (3c-i)** (AiReviewPanel client component — displays aiSummary in editable textarea + per-note moment titles in editable inputs; Save button PATCHes /package; success/error/saving states; guardrail: AI assists never invents, coach explicitly reviews before approval; 7 tests, 688 total). Next: Sub-slice 3c-ii — approve button + wire panel into lesson page.
...
**Wave 4 — AI (Deepgram + OpenAI).** All API routes complete + coach AI review panel component complete: transcription adapter + worker route, packaging adapter + worker route, coach edit AI output (PATCH /package), coach approval route (POST /approve), and **AiReviewPanel client component (3c-i)** — 688 tests. Commit ee613b2.

**Next:** Sub-slice 3c-ii — (1) add an "Approve & Send Lesson" button to the AiReviewPanel (or a sibling component) that calls POST /api/submissions/[id]/approve, showing a success state when the manifest transitions to "approved". (2) Wire the AiReviewPanel into the coach's lesson page (`app/coach/submission/[id]/lesson/page.tsx`) — load the playback manifest server-side (via `getPlaybackManifestForSubmission`) and render `<AiReviewPanel>` when `manifest.aiSummary` exists. The existing `LessonApprovalForm` (old Phase 7 draft flow) may coexist or be superseded; keep both for now, gate the new panel on manifest existence. Follow the established client-component testing pattern (render/smoke test for the approve button + success/error states).
...
**Last Updated:** 2026-06-22 — Wave 4 (AI) IN PROGRESS. Coach AI review panel COMPLETE (3c-i): `AiReviewPanel` client component displays aiSummary (editable textarea) + per-note aiNoteTitles (editable inputs); Save button PATCHes /api/submissions/[id]/package; success/error/saving states. Guardrail: AI assists never invents — coach explicitly reviews/edits before approval. 7 new tests (688 total). Commit ee613b2. Next: Sub-slice 3c-ii — approve button + wire panel into lesson page.
