...
**Started:** 2026-06-21
**Status:** Wave 3 in progress — beforeunload unsaved-changes warning COMPLETE. 581 tests. Next: Wave 3 remaining polish (processing state visual indicator, or transition to Wave 4 AI/transcription).
...
**Wave 3 — Review Studio polish.** Beforeunload unsaved-changes warning COMPLETE: added `useBeforeUnloadWarning(shouldWarn)` hook that registers a `beforeunload` listener triggering the browser's "Are you sure you want to leave?" dialog when the coach has unprocessed notes (notes.length > 0 && !lessonUrl && !processing). The autosave localStorage draft is the recovery mechanism — if the coach confirms leaving, their notes are already persisted and restored on return. 10 new tests (7 hook unit tests + 3 render/smoke wiring tests). 581 total. Commit 4972ee4.

**Next: Wave 3 is nearly complete.** Remaining Wave 3 items to consider:
1. **Processing state visual indicator** — a small "Saving..." / "Saved" badge or spinner during the processLesson fetch, beyond the current "Processing..." button label. Low priority — the button already shows processing state.
2. **Transition to Wave 4 (AI)** — Deepgram transcription worker + OpenAI packaging. This is the next major functional wave. The transcript fields (transcriptRaw, transcriptEdited, transcriptStatus) are already in place from Wave 1 types extension. The env-gated adapter pattern (storage → payment → email → transcription) is well-established.

**Recommendation:** Wave 3 Review Studio polish is functionally complete (autosave, re-record, transcript edit, video error recovery, annotation fallback, retake thumbnail, thumbnail zoom, mobile touch targets, annotation toolbar mobile layout, beforeunload warning). Next tick should start Wave 4 (AI/transcription) or pick up a minor Wave 3 visual polish item if the user wants more studio refinement first.
...
**Last Updated:** 2026-06-22 — Wave 3 beforeunload unsaved-changes warning COMPLETE. Added `useBeforeUnloadWarning(shouldWarn)` hook + wired into ReviewStudioClient (shouldWarn = notes.length > 0 && !lessonUrl && !processing). Browser's "Are you sure you want to leave?" dialog prevents accidental tab-close/navigation mid-review. Autosave localStorage is the recovery mechanism. 10 new tests (7 hook unit + 3 render/smoke wiring), 581 total. Commit 4972ee4. Next: Wave 3 is functionally complete — transition to Wave 4 (AI/transcription) or minor visual polish.
