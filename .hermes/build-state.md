...
**Started:** 2026-06-21
**Status:** Wave 4 (AI) IN PROGRESS — transcription adapter layer + transcription worker route + OpenAI packaging adapter layer + packaging worker route + coach edit AI output (PATCH /package) + coach approval route (POST /approve) COMPLETE. 681 tests. Next: coach-facing UI for reviewing AI output + approve button (Sub-slice 3c).
...
**Wave 4 — AI (Deepgram + OpenAI).** All API routes complete: transcription adapter + worker route, packaging adapter + worker route, coach edit AI output (PATCH /package), and **coach approval route (POST /approve)** — 681 tests. Commit bbd5839.

**Next:** Sub-slice 3c — coach-facing UI for reviewing AI output + approve button. This is a client component (or page section) that: (1) fetches the packaged manifest (GET playback manifest + aiSummary/aiNoteTitles), (2) displays the AI summary and per-note titles for coach review, (3) lets the coach edit them (calls PATCH /api/submissions/[id]/package), (4) shows an "Approve & Send Lesson" button that calls POST /api/submissions/[id]/approve. Decompose: 3c-i = review UI component (display AI output + edit), 3c-ii = approve button + wiring. Follow the established client-component testing pattern (mock browser-API children, render/smoke test).
...
**Last Updated:** 2026-06-22 — Wave 4 (AI) IN PROGRESS. Coach approval route COMPLETE: `POST /api/submissions/[id]/approve` — transitions manifest status to "approved" + triggers delivery token + email on first approval. Idempotent on re-approve. 409 if manifest not packaged. Non-fatal side effects. 9 new tests (681 total). Commit bbd5839. Next: Sub-slice 3c — coach-facing UI for reviewing AI output + approve button.
