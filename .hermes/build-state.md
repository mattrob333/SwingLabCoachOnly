...
**Started:** 2026-06-21
**Status:** Wave 3 in progress — annotation toolbar mobile layout fix COMPLETE. 571 tests. Next: Wave 3 remaining mobile polish (processing state + safe recovery, or move to next Wave 3 item).
...
**Wave 3 — Review Studio polish.** Annotation toolbar mobile layout fix COMPLETE: removed the `max-sm:top-[calc(100%+0.5rem)]` positioning that placed the toolbar BELOW the video frame (overlapping with the scrubber bar and playback controls). The toolbar now stays overlaid on the video at bottom-left on ALL screen sizes. To minimize video coverage on narrow screens, Undo/Clear buttons are now icon-only on mobile (text labels wrapped in `hidden sm:inline` spans) and the marks-count text is hidden on mobile (`hidden sm:inline`). 4 new tests (571 total). Commit 121db6f.

**Next: Wave 3 remaining polish.** Pick the smallest next slice:
1. **Mobile responsiveness** — video player controls ✅, note card touch targets ✅, lightbox close button ✅, annotation canvas toolbar touch targets ✅, annotation toolbar mobile layout ✅ DONE (this tick). Next mobile item: processing state + safe recovery if coach leaves mid-review.
2. **Processing state + safe recovery** — the autosave feature already restores drafts on mount, but there's no explicit "processing" indicator or unsaved-changes warning when navigating away mid-review. This is the last major Wave 3 item.
3. **Thumbnails** — retake ✅, zoom ✅.
...
**Last Updated:** 2026-06-22 — Wave 3 annotation toolbar mobile layout fix COMPLETE. Removed the `max-sm:top-[calc(100%+0.5rem)]` positioning that placed the toolbar BELOW the video frame (overlapping with the scrubber bar and playback controls). The toolbar now stays overlaid on the video at bottom-left on ALL screen sizes. To minimize video coverage on narrow screens, Undo/Clear buttons are now icon-only on mobile (text labels wrapped in `hidden sm:inline` spans) and the marks-count text is hidden on mobile (`hidden sm:inline`). 4 new tests, 571 total. Commit 121db6f. Next: Wave 3 remaining polish (processing state + safe recovery, or next polish item).
