...
**Auth login+logout API route tests ✅ DONE (commit eb155ee):** 17 tests covering the two most security-critical API routes that previously had zero API-level test coverage. Login: malformed JSON → 400, missing slug/password → 400, unknown slug → 401 (no-leak: same error as wrong password), valid credentials → 200 + cookie with HttpOnly/SameSite=Lax/Path=/Max-Age, Secure flag in production, token verifiable, slug trimming. Logout: redirect + cookie clear (Max-Age=0) + security attrs. 951 tests total.

**Next: Wave 6 Task 5 continued (additional API error-handling coverage gaps — e.g. comparison-api, earnings-api, pay-api may lack error/edge-case tests) OR UX polish task #10 (accessibility pass).** Interleave as needed.
