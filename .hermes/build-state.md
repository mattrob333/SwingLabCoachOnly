...
**Started:** 2026-06-21
**Status:** Wave 4 (AI) IN PROGRESS — transcription adapter layer + transcription worker route + OpenAI packaging adapter layer + packaging worker route COMPLETE. 661 tests. Next: coach approval flow (review AI-generated titles/summary, edit, approve → delivers).
...
**Wave 4 — AI (Deepgram + OpenAI).** Transcription adapter layer + transcription worker route + OpenAI packaging adapter layer + packaging worker route ALL COMPLETE. The packaging worker route (`POST /api/submissions/[id]/package`) authenticates coach, verifies ownership, loads the stored `LessonPlaybackManifest`, calls `getPackagingAdapter().package()`, and persists the result (`aiSummary` + `aiNoteTitles`) back onto the manifest. 8 new tests (661 total). Commit 565b996.

**Next:** Coach approval flow (Wave 4 Task 3) — the coach reviews the AI-generated titles/summary, can edit them, and approves → triggers delivery (create delivery token + send email, reusing the Wave 2 approve→deliver wiring). This involves: (a) a UI surface for the coach to review/edit AI output (could be a new page or extend the existing lesson-draft/review page), (b) a PATCH/POST route to save edited titles/summary and transition to "approved", (c) wiring the approval to the existing delivery token creation + email send from Wave 2 Sub-slice C. Decompose into sub-slices: 3a = API route to save edited AI output, 3b = API route to approve (transition + delivery), 3c = coach-facing UI.
...
**Last Updated:** 2026-06-22 — Wave 4 (AI) IN PROGRESS. Packaging worker route COMPLETE: `POST /api/submissions/[id]/package` — auth + ownership + load manifest + call `getPackagingAdapter().package()` + persist `aiSummary` + `aiNoteTitles` onto manifest. Adds `aiNoteTitles` field to `LessonPlaybackManifest`. 8 new tests (661 total). Commit 565b996. Next: coach approval flow (review/edit AI output, approve → delivery).
