...
**Supabase impls:** The four Supabase repository implementations (`SupabaseSubmissionRepository`, `SupabaseCoachRepository`, `SupabaseEarningRepository`, `SupabasePlaybackManifestRepository`) make real PostgREST `fetch()` queries against the Supabase REST API (`/rest/v1/<table>`) using the service-role key (bypasses RLS — server-side only). They map between Postgres row shapes (snake_case, INTEGER cents, ISO timestamps) and TypeScript domain types (camelCase, dollar amounts, Date objects). Tests in `tests/repositories/supabase-impls.test.ts` mock `global.fetch` to verify correct PostgREST calls and row mapping without a live database.
